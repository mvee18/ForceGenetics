...
var FitnessLimit int64 = 1000

func main() {
	start := time.Now()
	rand.Seed(time.Now().UTC().UnixNano())
	target := load("./space.png")
...
func createRandomImageFrom(img *image.NRGBA) (created *image.NRGBA) {
...
// save the image
func save(filePath string, rgba *image.NRGBA) {
...
func diff(a, b *image.NRGBA) (d int64) {
	d = 0
...
func createPool(population []Organism, target *image.NRGBA) (pool []Organism) {
...
func naturalSelection(pool []Organism, population []Organism, target *image.NRGBA) []Organism {
...
func createPopulation(target *image.NRGBA) (population []Organism) {
...
func createOrganism(target *image.NRGBA) (organism Organism) {
	organism = Organism{
...
func (o *Organism) calcFitness(target *image.NRGBA) {
